...
def init(path, is_cuda):
    ...
    data, labelDictionary = audio_manager.generate_dataset(
        path, wanted_words, available_words)

    training_set, testing_set, validation_set = audio_manager.split_data_set(
        data, .80, .10, .10)
    ...
    testing_list, testing_label_list = audio_manager.convert_to_minibatches(
        testing_set, 1)

    # needed to reshape/organize validation set:
    validation_set = audio_manager.feature_extraction(validation_set)
    validation_list, validation_label_list = audio_manager.convert_to_minibatches(
        validation_set, 1)
    ...
    for epoch_num in range(num_epochs):
        print('Training on epoch #', epoch_num, ' time: ',
        ...
        mini_batch_list, mini_batch_label = audio_manager.convert_to_minibatches(
            train_batch, 100)
