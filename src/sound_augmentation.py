...
def add_background_noise(input_audio, background_audio):
    ...
    add_background_noise = True if random.uniform(
        0, 1) > 0.2 else False  # 80% of audio is augmented
    if add_background_noise:
        chosen_background_audio = background_audio[random.randint(
            0, len(background_audio)-1)]
        random_noise_level = random.uniform(0, 0.1)
        result = (1 - random_noise_level) * input_audio + \
            random_noise_level * chosen_background_audio[:len(input_audio)]
    else:
        result = input_audio
    return result
...
def shift(input_audio, test=False):
    ...
    timeshift_fac = np.random.uniform(
        0, 0.10) if not test else 0.5  # up to 10% of length
    start = int(input_audio.shape[0] * timeshift_fac)
    if (start > 0):
        result = np.pad(input_audio, (start, 0), mode='constant')[
            0:input_audio.shape[0]]
    else:
        result = np.pad(input_audio, (0, -start),
                        mode='constant')[0:input_audio.shape[0]]
    return result
